...
export async function POST(request: Request) {
  try {
    ...
    // Get all Gmail integrations (supports multiple accounts)
    ...
    for (const integration of integrations) {
      ...
      console.log(`Gmail sync (${integration.metadata?.email || 'account'}): ${deepSync ? 'DEEP (90 days)' : 'Regular (7 days)'}`);
      ...
      // Update last sync time for this specific account
      await supabase
        .from('integrations')
